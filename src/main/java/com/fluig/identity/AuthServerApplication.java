        ...
        return new CommandLineRunner() {
            ...
            public void run(String... arg0) throws Exception {
                ...
                if (!jdbcUserDetailsManager.userExists("user")) {
                    jdbcUserDetailsManager.createUser(new User("user", "{noop}user", Lists.newArrayList(new SimpleGrantedAuthority("READ"))));
                }
                if (!jdbcUserDetailsManager.userExists("admin")) {
                    jdbcUserDetailsManager.createUser(new User("admin", "{noop}admin", Lists.newArrayList(new SimpleGrantedAuthority("READ"), new SimpleGrantedAuthority("WRITE"))));
                }
                if (!jdbcUserDetailsManager.userExists("user1")) {
                    for (int i = 1; i <= 1000; i++) {
                        if (i % 2 == 0) {
                            jdbcUserDetailsManager.createUser(new User("user" + String.valueOf(i), "{noop}user" + String.valueOf(i), Lists.newArrayList(new SimpleGrantedAuthority("READ"))));
                        } else {
                            jdbcUserDetailsManager.createUser(new User("user" + String.valueOf(i), "{noop}user" + String.valueOf(i), Lists.newArrayList(new SimpleGrantedAuthority("READ"), new SimpleGrantedAuthority("WRITE"))));
                        }
                    }
                }
                if (!jdbcUserDetailsManager.userExists("wcmadmin")) {
                    jdbcUserDetailsManager.createUser(new User("wcmadmin", "{noop}adm", Lists.newArrayList(new SimpleGrantedAuthority("READ"), new SimpleGrantedAuthority("WRITE"))));
                }
